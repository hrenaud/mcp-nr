...
>
> **Remédiation terminée** (branche `fix/infra-parity-and-auth-failsafe`). **28/51 constats traités** (Tasks 1 à 11 du plan + convergence infra & fail-safe auth hors review). **0 constat actionnable restant.** Les 23 autres se répartissent en 15 faux positifs / non-actions justifiées et 8 informatifs. Voir [§7.1](#71-statistiques) et [§8](#8-travaux-additionnels-hors-review).
...
| ---- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| ✅ 1 | auth.py:104-133 | **`DynamicTokenVerifier.__init__`** : `charger_tokens()` échoue silencieusement si le fichier est corrompu (`json.load` lève une exception → logger.error → retourne `{}`). Le verifier est alors vide → **aucun token valide**. _(Fait Task 9 : le `logger.error` existait déjà (pas totalement silencieux) ; comportement verrouillé par un test de régression. Combiné au fail-safe auth, un fichier corrompu en HTTP empêche désormais de servir au lieu d'ouvrir sans token.)_ | Moyen   |
| ✅ 2 | auth.py:98-101  | **Import dynamique `AccessToken = None`** : si `fastmcp.server.auth.AccessToken` est introuvable au runtime, `DynamicTokenVerifier.verify_token()` (ligne 127) fait `AccessToken(...)` → **`TypeError: 'NoneType' object is not callable`**. _(Fait Task 8 : `RuntimeError` explicite si `AccessToken is None`.)_                                                                                                                                                                   | Moyen   |
| ✅ 3 | auth.py:196-208 | **`update()`** ne valide pas `name` (peut être chaîne vide) ni `expires_days` (`<= 0` passe sans rejection). **No-input de validation**. _(Fait Task 8 : `ValueError` si `name` vide ou `expires_days <= 0`.)_                                                                                                                                                                                                                                                                      | Faible  |
| 🟦 4 | auth.py:160-177 | **`list_all()`** et **`get_by_id()`** : scan linéaire O(n) sur **tous** les tokens du fichier. Si 10 000 tokens, chaque appel est lent. _(Non-action : YAGNI — quelques dizaines de tokens en pratique, pas d'index spéculatif.)_                                                                                                                                                                                                                                                   | Faible  |
...
| #    | Fichier:Ligne     | Problème                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Gravité  |
| ---- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| 🟦 5 | routes.py:533     | **`_http_homepage`** importe `charger_cache` de `data` **en runtime** (à l'intérieur de la fonction). Si `data.py` n'est pas dans le module path → **`ImportError`**. _(Non-action : voulu — import différé = mécanisme d'injection ; `core` ne dépend pas du `data.py` d'un MCP au top-level.)_                                                                                                                                                                                          | Faible   |
| ✅ 6 | routes.py:77-527  | **Template d'installation shell** : `.replace("__MCP_ID__", mcp_id)` injecte une variable utilisateur dans du bash. Si `mcp_id` contient `__` (ex: `my__mcp`), `.replace("__BASE_URL__", ...)` peut être corrompu → **injection de code arbitraire dans le script**. (IDs actuels "greenit"/"rgaa"/"rgesen" sont sûrs.) _(Fait Task 7 : `_http_install_script` valide `_MCP_ID` (`[a-z][a-z0-9-]*`) avant injection.)_                                                                    | Moyen    |
...
| #     | Fichier:Ligne           | Problème                                                                                                                                                                                                                                                                                                                                                                                                        | Gravité  |
| ----- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| 🟦 12 | greenit_mcp.py:62       | **`_REFERENTIEL_VERSION` calculé au load time** : `charger_cache().get("meta", {}).get("version", "")`. Si le cache est vide au démarrage → version vide. **Ne se met pas à jour** après un reload. _(Non-action Task 10 : le cache est figé dans l'image Docker, jamais rechargé à chaud → lecture au load time acceptable. Clos.)_                                                                            | Faible   |
...
| ✅ 14 | greenit_mcp.py:143-808  | **Double `output_schema`** : chaque outil a `@mcp.tool(output_schema=...)` ET une définition dans `routes._greenit_tool_definitions()`. **18 points de failure** au lieu de 9. _(Fait Task 3 : la table manuelle est supprimée, le /guide est dérivé des décorateurs. NB : `output_schema=` (contrat de sortie) reste sur le décorateur — c'était la table d'entrée `_tool_definitions` la vraie duplication.)_ | **Haut** |
...
| 🟦 20 | rgaa_mcp.py:44       | **`_REFERENTIEL_VERSION` calculé au load time** comme GreenIT. Idem. _(Non-action Task 10 : idem #12 — clos.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Faible   |
| ✅ 21 | rgaa_mcp.py:59-180   | **Double `output_schema`** : chaque outil RGAA a `@mcp.tool(output_schema=...)` ET `_rgaa_tool_definitions()`. **20 points de failure**. _(Fait Task 3 : table manuelle supprimée, /guide dérivé des décorateurs.)_                                                                                                                                                                                                                                                                                                                                                                                     | **Haut** |
| 🟦 22 | analyseur.py:15-19   | **`fetcher_html`** utilise `httpx.Client(timeout=30, follow_redirects=True)` avec `User-Agent: "Mozilla/5.0 RGAA-MCP/1.0"`. **Pas de User-Agent rotation**. Possible blocage par certains serveurs. _(Non-action : hors périmètre — un analyseur d'accessibilité ne doit pas masquer son identité.)_                                                                                                                                                                                                                                                                                                    | Faible   |
| ✅ 23 | analyseur.py:59-73   | **Thème 1 (images)** : ne vérifie **que** `alt` manquant. Ne détecte **PAS** : images décoratives sans `role="presentation"` ou `aria-hidden="true"`, images SVG sans `<title>`, `<input type="image">` sans `alt`. **Faux-NC** sur images décoratives actives. _(Fait Task 6 : limite documentée dans le champ `note` des résultats (détection inchangée, par conception d'une analyse statique).)_                                                                                                                                                                                                    | Moyen    |
| ✅ 24 | analyseur.py:183-204 | **Thème 9 (titres)** : scan de **TOUT** le HTML pour `<h1>`-`<h6>`, puis vérifie sauts de niveau consécutifs. Si un `<h3>` apparaît dans un aside après un `<h1>` dans le body, le test considère cela comme un **saut** h1→h3. **Faux-NC** sur pages multi-sections. _(Fait Task 6 : limite documentée dans le champ `note` (faux-NC multi-sections), à vérifier manuellement.)_                                                                                                                                                                                                                       | Moyen    |
| ✅ 25 | analyseur.py:236-247 | **Thème 12 (skip links)** : cherche `a[href^="#"]` dont le texte contient "contenu", "navigation", etc. Si un lien ancre (`#top`) a un texte différent, **faux-NC**. **AUCUN test** dans `test_analyseur.py` pour ce thème. _(Fait Task 4 : tests 12.11 C/NC ajoutés. (La limite « faux-NC ancres » relève de la précision d'analyse, cf. #24/Task 6.))_                                                                                                                                                                                                                                                | **Haut** |
| ✅ 26 | analyseur.py         | **Pas de test pour theme 5.7** (table header `scope`). **Pas de test pour theme 12** (skip links). **Pas de test pour theme 8.5** (`<title>` contenu non vide) et **8.6** (meta charset). _(Fait Task 4 : classe `TestCouvertureManquante` couvre 5.7, 8.5, 8.6, 12.11.)_                                                                                                                                                                                                                                                                                                                               | **Haut** |
...
| 🟦 28 | rgesn_mcp.py              | **Moins de prompts** (7 vs 8 GreenIT / 11 RGAA). Coverage des workflows plus limitée. _(Non-action : informatif — couverture de prompts, pas un défaut.)_                                                                                      | Faible  |
...
| 🟦 30 | rgesn_mcp.py              | **`_REFERENTIEL_VERSION` calculé au load time** comme les autres. _(Non-action Task 10 : idem #12 — clos.)_                                                                                                                                    | Faible  |
| ✅ 31 | rgesn/tests/test_rgesn.py | **Manque de tests de coverage** : les 7 tests de `test_rgesn.py` couvrent les fonctionnalités de base mais **pas les cas limites** (fiches inexistantes, paramètres invalides, etc.). _(Fait Task 4 : test critère inexistant → `ToolError`.)_ | Moyen   |
...
| #     | Fichier:Ligne    | Problème                                                                                                                                                                                                                                                                                                                                                                                          | Gravité  |
| ----- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| 🟦 32 | release.sh:25-34 | **`preparer_donnees.py` n'est JAMAIS exécuté** : la release bump la version mais **ne met pas à jour les caches de données**. Les 3 MCPs tournent avec des données **périmées** depuis la dernière exécution manuelle de `preparer_donnees.py`. _(Non-action : voulu — le rafraîchissement des données est un acte manuel séparé du bump de version (sinon release dépendante du réseau amont).)_ | **Haut** |
| ✅ 33 | release.sh:47    | **`git add "${MCPS[@]/%//files/*_mcp.py}"`** : glob pattern `{name//pattern/replacement}` qui échoue silencieusement en bash ancien → fallback hardcodé. **Comportement différent entre bash versions**. _(Fait Task 7 : `git add` explicite, expansion fragile supprimée.)_                                                                                                                      | Moyen    |
| ✅ 34 | release.sh       | **Ne met à jour ni `CHANGELOG.md`, ni `.mcp.json`, ni `README.md`** : alors que CLAUDE.md indique "**règle changelog + tag avant chaque release**". _(Fait Task 7 : `release.sh` exige une entrée `## [version]` dans CHANGELOG.md et la committe. `.mcp.json` traité à part — secrets, cf. note bas de §7.2.)_                                                                                   | **Haut** |
| ✅ 35 | release.sh:19    | **Ne vérifie pas la branche courante** : peut tourner sur `feature/foo` et faire un commit+tag. **Devrait vérifier `git branch --show-current == "main"`**. _(Fait Task 7 : refus si branche ≠ main.)_                                                                                                                                                                                            | Moyen    |
...
| ✅ 37 | build.sh          | **Ne lance pas les tests** avant le build. `local-build.sh` oui, mais `build.sh` non. **Incohérence entre scripts**. _(Fait Task 7 : tests lancés avant build + commentaire d'en-tête corrigé.)_ | Faible  |
...
| #     | Fichier:Ligne | Problème                                                                                                                                                                                                                                                                                                                                                                                                                   | Gravité |
| ----- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| 🟦 38 | ci.yml:43-45  | **Les tests ne couvrent PAS l'analyseur RGAA** : `pytest rgaa/tests/` inclut `test_analyseur.py` qui teste `BeautifulSoup` (HTML statique). **L'analyseur en production** (si Playwright était utilisé pour DOM rendu) **ne serait PAS testé**. _(Faux positif : l'analyseur opère sur HTML statique (BeautifulSoup), aucun rendu Playwright — rien à tester côté DOM rendu ; `test_analyseur.py` couvre le chemin réel.)_ | Moyen   |
| ✅ 39 | ci.yml:67-68  | **Smoke test Docker** : `docker run --rm ${{ matrix.image }} --health`. **Ne vérifie PAS** que le serveur répond sur le port 8000. `--health` est un argument CLI, pas un HTTP health check. _(Fait Task 7 : conteneur démarré en HTTP + `curl` sur le port 8000.)_                                                                                                                                                        | Moyen   |
...
| ----- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| ✅ 46 | **Règle d'architecture** : "**Si deux MCPs ont du code quasi-identique → il doit aller dans core**" | **`output_schema`** redéfini dans les 3 MCPs (9 GreenIT + 10 RGAA = **19 définitions** en double). Devrait être dans `routes.py`. _(Fait Task 3 : introspection commune dans `core`. = #7/#14/#21/#51)_                                                       | **Haut** |
...
| ✅ 49 | **Règle** : "**Règle changelog + tag avant chaque release**"                                        | `release.sh` ne met **pas à jour** `CHANGELOG.md`. _(Fait Task 7 — = #34)_                                                                                                                                                                                    | **Haut** |
| ✅ 50 | **Règle** : "**Tests écrits avant implémentation (TDD)**"                                           | **Themes 5.7 et 12 de l'analyseur RGAA : AUCUN test**. Implémentés mais non couverts par les tests. _(Fait Task 4 — = #25/#26)_                                                                                                                               | **Haut** |
| ✅ 51 | **Règle** : "**Simplicité d'abord : pas de duplication**"                                           | **19 points de failure** pour `output_schema`. Un seul changement demanderait de modifier 3 fichiers. _(Fait Task 3 — = #46)_                                                                                                                                 | **Haut** |
...
| Statut                                     | Count | #s                                                                                                                                                                    |
| ------------------------------------------ | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ✅ **Fait**                                | 28    | 1, 2, 3, 6, 7, 11, 14, 16, 17, 18, 21, 23, 24, 25, 26, 31, 33, 34, 35, 37, 39, 41, 44, 46, 47, 49, 50, 51 (+ fail-safe auth & parité infra, **hors review** — cf. §8) |
| ⬜ **À faire**                             | 0     | —                                                                                                                                                                     |
| 🟦 **Faux positif / non-action justifiée** | 15    | 4, 5, 8, 9, 12, 13, 19, 20, 22, 28, 30, 32, 38, 45, 48                                                                                                                |
| ℹ️ **Info (rien à corriger)**              | 8     | 10, 15, 27, 29, 36, 40, 42, 43                                                                                                                                        |
